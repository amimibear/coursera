...
def main():
    key = random(1024)
#   ciphertexts = [encrypt(key, bytes.fromhex(msg)) for msg in MSGS]
    l = [bytes.fromhex(msg) for msg in MSGS]
    ...
    for k in range(10):
        ...
        for j in range(11):
            s = set()
            ll = strxor(l[k],l[j])
            for i,c in enumerate(ll.decode('ISO-8859-1')):
                if c>='A': # 空格会小写转大写，大写转小写 大写0100**** 小写0110**** 空格00100000
                    n[i] += 1
        for i in range(200):
            if n[i]>5 and i<len(l[10]):
                m[i].append(chr(l[10][i]^l[k][i]^32))
    for i in m[:85]:
        print(i)
